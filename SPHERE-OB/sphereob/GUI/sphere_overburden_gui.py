...
class OptionsMenu(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        coeff_grid.addWidget(self.read_data_btn, 7,0)
...
class AppForm(QtWidgets.QMainWindow):
    ...
    def readCSV(self):
        ...
        if fileName:
            self.xs = 1
            data = pd.read_csv(fileName, sep='\s+', skiprows=10, header=None, on_bad_lines='skip')
            data2 = data.dropna()
            TEM = data2.rename(columns=data2.iloc[0]).drop(data2.index[0])
            del TEM['CALCTHIS']
            lineID = TEM.NORTH.unique()
            print(lineID)
